...
with transaction.atomic():
    ...
    req=RequestFactory().get('/reports/sales/pdf/',{'from':'2024-01-01','to':'2024-01-31','period':period.pk,'marketer':user.pk});req.user=user
    for name,cols,qs in [('wide',['reference','description','amount','date','marketer','status'],Sale.objects.filter(marketer=user)),('portrait',['reference','amount','date'],Sale.objects.filter(marketer=user)),('empty',['reference','amount','date'],Sale.objects.none())]:
        data=build_pdf(req,'sales',qs,cols);(OUT/('pdf-'+name+'.pdf')).write_bytes(data);reader=PdfReader(BytesIO(data));text='\n'.join(p.extract_text() for p in reader.pages)
...
(OUT/'pdf-results.json').write_text(json.dumps(results,indent=2));print(results)
