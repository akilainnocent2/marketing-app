...
with sync_playwright() as p:
    ...
    page.goto('http://127.0.0.1:8000/accounts/login/');page.screenshot(path=str(OUT/'login-1536.png'),full_page=True)
    ...
    page.goto('http://127.0.0.1:8000/marketing/sales/');page.screenshot(path=str(OUT/'sales-list.png'),full_page=True)
    ...
    native.goto('http://127.0.0.1:8000/accounts/login/');native.fill('[name=username]',user);native.fill('[name=password]',password);native.click('button[type=submit]');native.wait_for_url('**/dashboard/')
    native.goto('http://127.0.0.1:8000/records/customers/add/');native.fill('[name=name]','No JavaScript');native.fill('[name=phone]','0612345678');native.fill('[name=comment]','Native POST');native.locator('.record-form button[type=submit]').first.click();native.wait_for_url('**/marketing/customers/');assert native.get_by_text('No JavaScript',exact=True).count()>0
